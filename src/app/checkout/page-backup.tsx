...
export default function CheckoutPage() {
  ...
  if (lines.length === 0) {
    return (
      <div className="container-vl py-32 text-center">
        ...
        <p className="text-obsidian/60 mt-3">Add a product to your bag first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container-vl py-16">
      <h1 className="font-display text-3xl md:text-4xl mb-12">Checkout</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="grid lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-10">
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <input {...register('fullName')} placeholder="Full name" className="input-field sm:col-span-2" />
              <input {...register('line1')} placeholder="Address" className="input-field sm:col-span-2" />
              <input {...register('city')} placeholder="City" className="input-field" />
              <input {...register('state')} placeholder="State / Province" className="input-field" />
              ...
              <input {...register('phone')} placeholder="Phone" className="input-field sm:col-span-2" />
            </div>
            {(errors.fullName || errors.line1 || errors.city) && (
              <p className="error-text">Please complete all required shipping fields.</p>
            )}
          </section>
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <input {...register('cardNumber')} placeholder="Card number" className="input-field sm:col-span-2" />
              <input {...register('cardExpiry')} placeholder="MM/YY" className="input-field" />
              <input {...register('cardCvc')} placeholder="CVC" className="input-field" />
            </div>
          </section>
        ...
        <div className="bg-blush/60 p-8 h-fit">
            ...
            {lines.map((l) => (
              <div key={l.productId} className="flex gap-3">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <p className="text-obsidian/50">${(l.price * l.quantity).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="border-t border-obsidian/15 mt-6 pt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between font-medium text-base pt-2"><span>Total</span><span>${total.toFixed(2)}</span></div>
          </div>
        </div>
      </form>
    </div>
